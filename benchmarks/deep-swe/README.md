...
- **12 skip in a tree that has `runs/`.** 10 need `pier`: 2 check that the cost override is what inheritance resolves to and that it still delegates to the pure rule, 3 cover the file-opening shell around that rule, 1 compares the override against pier's own implementation on the recorded stream, and 4 cover the `exec_as_agent` override that strips pier's gateway model-alias pins — that it is what inheritance resolves to, and that it reaches pier's own implementation with `env` filtered and `command`/`cwd`/`timeout_sec` intact. The 11th renders a prompt template through `jinja2`; the specific property it proves — that pier can render every arm's template with only `instruction` bound — is *also* checked by a stdlib equivalent that always runs, so that one property does not depend on jinja2 being installed. The 12th is `tests/test_stream_cost.py`'s fixture-versus-original drift check, whose source transcript is no longer among the recorded runs in this tree.
- **34 skip in a fresh clone**, because `.gitignore` is exactly `runs/`: the recorded evidence is untracked, so 22 more tests that read it skip too. The count is not merely shifted from run to skip: five tests do not enumerate at all without `runs/` (793 collected, against 798 with it). Neither tree is green: `python3 -m unittest discover` reports `failures=48, errors=2, skipped=12` with `runs/` present and `failures=16, errors=1, skipped=34` without it. Those failures are known and none of them is a triage rule — they come from committed inputs that the triage and cost logic does not touch: `schedule.yaml` declares `between_runs: 1h` and 10 skipped cells while `tests/test_schedule.py`, `tests/test_collect_task_cells.py` and `tests/test_run_scheduled.py`'s preview tests still pin 2h and 12, and the cost-table, report-table, complexity-chart and completion-gate expectations were written against an earlier, smaller set of recordings — `tests/test_collect_completion_gate.py` still enumerates `do-in-steps__sonnet-sonnet/cattrs-partial-structuring-recov__ZsbwRdJ`, a trial this tree's `runs/` no longer holds. Losing `runs/` is a visibility gap on top of that, not a correctness one. Two regression proofs in particular survive it, both because their evidence was copied into `tests/fixtures/`: 13 of `tests/test_stream_cost.py`'s 14 tests still run from the committed 3 KB fixture, including the 22-event, `$0.392`-first, `$26.530`-max assertions, and every `tests/test_triage.py` assertion about the `rate_limit_info.status` vocabulary and worst-fault-wins ordering runs from `recorded-rate-limit-events.txt`. Only the two fixture-versus-original drift checks, and `test_triage.py`'s `RecordedTriageTests`, need `runs/` itself.
