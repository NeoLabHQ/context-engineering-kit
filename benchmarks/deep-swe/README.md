...
4. **Authentication** for the `claude` process pier launches inside each trial's container. `run.py` does `os.environ.copy()` before shelling out to `pier run`, so whatever you `export` on the host reaches **pier's own process** — but that is not the same as reaching the container, and the difference matters for every variable in [Forwarded environment variables](#forwarded-environment-variables). The container's env is a closed literal that pier builds by name: `ANTHROPIC_API_KEY`, `ANTHROPIC_AUTH_TOKEN`, `ANTHROPIC_BASE_URL`, `CLAUDE_CODE_OAUTH_TOKEN`, `CLAUDE_CODE_MAX_OUTPUT_TOKENS`, plus the Bedrock and AWS block when Bedrock mode is on (`ClaudeCode.run()`, `pier/agents/installed/claude_code.py:1219-1229` and `:1231-1263`). Pier then drops whichever of those are empty (`:1269`, comment: "Remove empty auth credentials to allow Claude CLI to prioritize the available method") — so set only the credential for the method you're using and leave the others unset. **Anything not on that list stops at pier and never reaches the container unless the harness forwards it explicitly** via pier's `--ae` flag; `run.py`'s `AGENT_ENV` is where that happens, and [Forwarded environment variables](#forwarded-environment-variables) lists what it forwards.
   ...
   - **Gateway / proxy routing:**
     ...
     Pier's container network allowlist is decided in this order (`claude_code.py:187-197`, method `network_allowlist`): Bedrock mode is checked **first** and returns `.amazonaws.com`, so it wins even when `ANTHROPIC_BASE_URL` is also set; otherwise `ANTHROPIC_BASE_URL`'s own hostname when that is set; else `api.anthropic.com`. Whichever pier picks is not the final list here — `agent.py`'s `ClaudeCodeSadd.network_allowlist` appends `github.com` to it, so the container can clone the pinned plugin checkout.

     **A gateway base URL makes pier erase this benchmark's implementation-tier axis, and the harness undoes that — read this before trusting a gateway-routed run.** When `ANTHROPIC_BASE_URL` and `ANTHROPIC_MODEL` are both set, `ClaudeCode.run()` points all three model aliases and the sub-agent model at the orchestrator's model (`claude_code.py:1290-1295`, comment: "When using custom base URL, set all model aliases to the same model"):
     ...
     Every plugin arm's prompt is `/{skill} --model {impl} …` and the sadd skills dispatch their implementation, meta-judge and judge sub-agents at that bare tier name, so pinned, the two mixed arms run their sub-agents at the *orchestrator's* model: `opus-sonnet` becomes `opus-opus` and `sonnet-haiku` becomes `sonnet-sonnet`. Nothing in `results.json` would show it — an arm id records what was requested, not what ran. `ClaudeCodeSadd.exec_as_agent` therefore **removes** those four variables on the way to the container, leaving Claude Code to resolve the aliases itself; the rule and its evidence are in `model_alias_pins.py`, and it acts only when both of the two variables pier keys off are present, so a first-party `api.anthropic.com` run is untouched.

     **This assumes your gateway resolves the bare aliases `haiku`, `sonnet` and `opus`.** That is the one thing the harness cannot check for you. If yours does not — if it only accepts fully-qualified model ids — removing the pins gives you three unresolvable aliases instead of one wrong-but-valid model, and `model_alias_pins.py` is where to intervene: map each alias to the concrete id your gateway serves instead of dropping it.

### Forwarded environment variables

Pier has generic `--ae/--agent-env KEY=VALUE` and `--env-file <path>` flags for injecting arbitrary container env (`pier/cli/jobs.py:348-358` for `--ae/--agent-env`, `:491-498` for `--env-file`). `run.py` uses `--ae`, from the module-level `AGENT_ENV` dict that `build_pier_command` emits as one `--ae KEY=VALUE` pair per entry. It holds one fixed entry plus the host variables listed below; a host variable that is unset — or set to blank — is not emitted at all, so it never appears in the job's recorded `config.json`. A value that *is* set has its surrounding whitespace stripped before forwarding, so `export API_TIMEOUT_MS=' 600000 '` reaches the container as `600000`; interior whitespace (the newlines separating `ANTHROPIC_CUSTOM_HEADERS` pairs) is untouched.

| Variable | Where it comes from | Notes |
|---|---|---|
| `CLAUDE_CODE_PRINT_BG_WAIT_CEILING_MS` | fixed at `0` by `run.py` | Not a tuning knob. `--print` mode otherwise abandons live sub-agents after 600s and exits 0, which pier reads as a clean finish; see the constant's comment in `run.py` for the run that was lost to it. The host cannot override it. |
| `ANTHROPIC_CUSTOM_HEADERS` | your `export`, forwarded when set | Extra headers a gateway requires (routing tenant id, bearer token). Pier does not read this name, so without the forward it stops at pier's process. |
| `API_TIMEOUT_MS` | your `export`, forwarded when set | Claude Code's per-request timeout. Worth raising on a gateway that queues or retries upstream — a judged skill's turns are long. Pier does not read this name either. |
| `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC` | pier's `"1"`, overridable by your `export` | Container telemetry. Pier hardcodes `"1"`; export `0` to turn it back on. Unset on the host, nothing is emitted and pier's `"1"` stands — see below for why forwarding beats the hardcode. |

**Why the last row works, given that pier hardcodes it.** `--ae` values are merged **twice**, and the second merge is downstream of the hardcode:

| Where | What happens to `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC` |
|---|---|
| `claude_code.py:1265` | `build_process_env` merges `--ae` — your value lands |
| `claude_code.py:1269` | empty values dropped — `"0"` is non-empty, so it survives |
| `claude_code.py:1302` | `env[...] = "1"` — pier clobbers your value |
| `claude_code.py:1336`/`:1341` | `run()` calls `exec_as_agent(env=env)` |
| `base.py:320-323` | `_exec` does `merged_env.update(self._extra_env)` — **your value wins** |

`run()` has finished writing `:1302` before it calls `exec_as_agent`, and `exec_as_agent` delegates to `_exec` (`base.py:377`), so the `--ae` re-merge is unconditionally last. Leaving the variable unset is therefore how you keep pier's default: no `--ae` pair means nothing re-merges over `:1302`. The harness does not emit `=1` itself, which would pin the default against a future pier release that changes it.

The one thing `--ae` cannot do for you:

- **`--ae` can set a variable, never delete one.** That is why the alias pins above are removed in `exec_as_agent` rather than through `AGENT_ENV`: `--ae ANTHROPIC_DEFAULT_HAIKU_MODEL=` would put an empty string in the container env, which is not the same as the variable being absent. Removal is the *only* reason to reach for that hook — anything needing a different **value** than the one `run()` wrote belongs in `FORWARDED_HOST_ENV`, as the row above does.

> **`ANTHROPIC_CUSTOM_HEADERS` is recorded in plaintext.** Pier redacts env values by *key name*, matching `(KEY|SECRET|TOKEN|PASSWORD|CREDENTIAL|AUTH)` (`pier/utils/env.py:5-7`, used by `templatize_sensitive_env` and `sanitize_env_assignment`). `ANTHROPIC_CUSTOM_HEADERS` matches none of those, so if your header carries a bearer token, that token is written verbatim into `runs/<arm-id>/config.json` and `lock.json` — and echoed by `run.py --dry-run`. `.gitignore` is exactly `runs/`, so it is not committed, but it is plaintext on local disk.

Adding another variable is a one-line change: append its name to `run.py`'s `FORWARDED_HOST_ENV`. Nothing else needs editing — `agent_env_for_host` picks it up, and `tests/test_run_agent_env.py` covers the set-versus-unset behaviour.

## How it works, briefly

`run.py` builds a matrix of **10 plugin arms**: 2 skills (`do-and-judge`, `do-in-steps`) × 5 orchestrator/implementation model-tier pairs (haiku/haiku, sonnet/haiku, sonnet/sonnet, opus/sonnet, opus/opus), plus **3 vanilla control arms** (haiku, sonnet, opus orchestrating themselves with no plugin, no slash command) when `--with-vanilla` is passed — 13 arms total. Pass `--skill do-and-judge` or `--skill do-in-steps` to restrict `--mode single/sample/full` to that one skill's 5 tier-pair arms instead of both skills' 10 (8 instead of 13 with `--with-vanilla` — vanilla arms aren't tied to a skill, so `--skill` never drops or duplicates them). Pass `--model haiku`, `--model sonnet`, or `--model opus` to restrict to that one *symmetric* tier pair — same tier orchestrating and implementing — instead of all 5 CELLS (1 arm per skill in play instead of 5); combine `--skill` and `--model` to run exactly one arm. Unlike `--skill`, `--model` **also** restricts `--with-vanilla`'s controls to that one tier (1 instead of 3), since vanilla arms are per-model rather than per-skill — leaving all 3 in would silently reintroduce the other two tiers a `--model` filter was meant to exclude. Each arm is one `pier run` invocation. `agent.py` (`ClaudeCodeSadd`) teaches pier how to load this plugin: it checks out this repo's `plugins/sadd` (pinned at `v3.8.1`) into the container and passes `--plugin-dir`, so a preflight failure here almost always means the plugin didn't load, not that a task failed. It also keeps the two *mixed* tier pairs honest behind a gateway base URL, where pier would otherwise collapse them onto their orchestrator's model — see [Forwarded environment variables](#forwarded-environment-variables) and the gateway bullet above.

Every arm writes to `runs/<arm-id>/` (job-level `arm.json`/`result.json`/`job.log`, plus one subdirectory per trial). `collect.py` walks that tree into `results.json`/`results.csv`; `report.py` turns those into `report.html`. `--mode scheduled` additionally writes two files of its own at `runs/` depth 1, beside the arm directories: [`scheduler-state.json`](#runsscheduler-statejson) (its resume record) and [`scheduler-attempts.jsonl`](#runsscheduler-attemptsjsonl) (its append-only per-attempt decision trail).
...
```
[schedule] schedule.yaml: 45 planned run(s), 35 runnable, 10 skipped by rule
  ...
  pacing: 35 still to run, 3600s between runs => 122400s (34.0h) of pacing alone, excluding run time.
  retries: at most 2 technical retries per run, 7200s backoff each => at most 105 executions and 504000s (140.0h) of extra backoff.
```

(`still to run` counts what a *fresh* tree would execute. In a tree that already has recordings under `runs/`, the preview subtracts the cells those cover, so the number is lower — 16, in the tree this was measured from.)
...
| **Fact** — runnable after `skips` (10 cells are deliberately excluded) | 35 |
| **Fact** — pacing alone, 1h between the 35 runnable cells (deterministic: `between_runs: 1h` × 34 gaps) | 122,400s (34.0h) |
| **Assumption** — run time per cell, carried over from [Time](#time) above's ~45 min/trial blended estimate (this mode runs exactly one task per cell, i.e. one trial) | ~26.25h (35 × 45 min) |
| **Assumption + Fact** — baseline total with zero technical failures (pacing + run time) | ~60.25h (**~2.5 days**) |
| **Fact** — worst-case EXTRA backoff if every runnable cell exhausts its retries (deterministic: 35 cells × 2 retries × 2h backoff) | 504,000s (140.0h) |
| **Assumption + Fact** — worst-case total (baseline + worst-case backoff + up to 2 extra run-time attempts per cell) | up to ~253h (**~10.5 days**) |
...
`n_incomplete` and `n_errored` are both reported per arm, and never summed: an `incomplete` trial is the agent's own abandonment and counts against the arm, while an `errored` one is the harness's fault and is dropped from the numbers entirely (so nothing silently disappears either way). Keeping `incomplete` in the denominator is deliberate — excluding it would let an arm raise its Pass@1 by walking away from the tasks it was losing. A verifier-certified success is never downgraded — not to `incomplete` by the completion gate, and not to `errored` by a pier exception that killed the process *after* the verifier had already scored the trial (a real recorded case: `runs/do-in-steps__opus-opus__abs-stepped-slices` earned `reward 1` and was then killed by an Anthropic 429). `errored` means "this trial is not a measurement", and a trial the verifier scored is one. An infra failure is likewise never relabelled `incomplete`. See `classify_status()` in `collect.py` for the full precedence table.
...
That is the project's test command; it is `cd benchmarks/deep-swe && uv run python3 -m unittest discover` with the dependency environment already prepared, and it is what CI runs. Invoking the plain command directly works identically (`python3 -m unittest discover -s benchmarks/deep-swe` from the repo root, if you have the deps). The whole suite is stdlib `unittest` and runs in about a second.

**The suite is green, and behaves identically with and without `runs/`.** `mise run verify` reports `Ran 806 tests` / `OK` — no failures, no errors, and no skips — and a copy of this tree with `runs/` removed reports exactly the same. That equivalence is deliberate, not incidental: `.gitignore` is exactly `runs/`, so the recorded evidence is untracked and a fresh clone does not have it. Tests that read it would either skip there (proving nothing) or pin figures that move whenever another trial is recorded — and this suite had both problems, which is why no test reads `runs/` any more.

What replaced it, in the two shapes the problem came in:

- **Logic that was merely being exercised *through* recorded data** now runs against constructed fixtures. Status classification, triage reason mapping, cell/partition math, schedule expansion, the preview renderer and the collect→report pipeline are all rules; they are pinned on inputs built to state each rule, so a fixture's expected values follow from its own declared shape rather than from how many trials have been run. `tests/collect_fixtures.py`'s `write_runs_tree` builds a small, structurally complete pier jobs directory for the cases that genuinely need a tree on disk.
- **Evidence that only a real recording could supply** was copied, verbatim, into `tests/fixtures/` (see that directory's README for the provenance of each file and how to regenerate it). Three corpora live there now: the cost stream's `result` events, the five rate-limit event shapes, and — added for this change — `recorded-final-messages.txt`, the closing prose of every trial recorded to date, which is what the completion gate's question heuristic is validated against. These are real agent output nobody composed for the tests, which is exactly what makes them worth keeping.

One check did not survive, and is worth naming rather than quietly dropping: `tests/test_readme_claims.py` used to re-derive the [cost table](#cost-and-time--read-this-before---mode-full)'s three rows from the artifacts under `runs/` and compare them figure by figure. That was a documentation-accuracy check against unreproducible measurements, not a unit test, and detaching it from `runs/` could not preserve its meaning — rebuilt on a fixture it would only assert that a number the fixture states matches a number the README states. **So nothing now verifies that those three dollar figures still match the streams they were read from**; what remains checks that the table is present, that each row's stated verdict follows from its own first→last figures, and that the bounded generalization drawn from it is still bounded. The rule underneath it — that `collect.py` reports the cost pier recorded rather than re-deriving it — is pinned directly, on a staged trial whose two figures deliberately disagree.

Missing an optional dependency still fails nothing of its own. Under an interpreter without `pier` or `jinja2`, `python3 -m unittest discover` reports `Ran 806 tests` / `OK (skipped=11)`: 10 need `pier` — 2 check that the cost override is what inheritance resolves to and that it still delegates to the pure rule, 3 cover the file-opening shell around that rule, 1 compares the override against pier's own implementation, and 4 cover the `exec_as_agent` override that strips pier's gateway model-alias pins — and the 11th renders a prompt template through `jinja2`, a property *also* checked by a stdlib equivalent that always runs. None of the 11 is a rule this harness decides anything by; that is the point of the split described below.

Run `uv run --with pytest python3 -m pytest tests/ -q` to execute the same suite under pytest (`806 passed, 535 subtests passed`). Every count in this section is hand-maintained — re-measure by running the commands, not by arithmetic.
...
Judgment logic is deliberately kept out of `agent.py` for this reason. Anything importing `pier` is unreachable under the default command above, so a rule living there would have tests that *skip* rather than run — a green suite proving nothing. The cost rule therefore lives in `stream_cost.py` (stdlib `json` only) with `tests/test_stream_cost.py` covering it unconditionally, and `ClaudeCodeSadd._parse_total_cost_from_stream_json` is reduced to opening the file and calling it. The gateway alias-pin rule follows the same split — `model_alias_pins.py` (no imports at all) with `tests/test_model_alias_pins.py` covering it unconditionally, and `ClaudeCodeSadd.exec_as_agent` reduced to calling it. Same split `collect.py` already uses for `plugin_load_error_from_init_event` / `incompleteness_reason_from_signals`. `tests/test_status_contract.py` closes the other end, deriving its expectations from `typing.get_args(collect.Status)` so a fifth trial status cannot be added without wiring it through `ArmAggregate` and `report.py`'s arm table. `tests/test_readme_claims.py` does the same for this file: it checks that the cost table is internally consistent and that the claim drawn from it is still bounded (see [Running the tests](#running-the-tests) for what that check no longer covers), that the quoted transcript line is verbatim against the committed prose corpus, that the `error_reason` values line up with the code in both directions (every reason the code emits is named here, and every reason named here — including the templated `pier_exception:<category>:<type>` form — is one the code can produce), and that no superseded "nothing below is measured" disclaimer has crept back in — the two documentation defects this harness actually shipped. It does not check the test counts in this section; those are maintained by hand. Five files cover `run.py`, importing it via `tests/run_fixtures.py` (which stubs the `agent` module when `pier` isn't installed): `tests/test_run_dispatch.py` pins the preflight dispatch predicate (`has_subagent_dispatch`) — the tool-name matcher that decides whether a sub-agent was ever dispatched, there because that predicate silently broke on a claude-code tool rename (`Task` → `Agent`) while the whole suite stayed green — `tests/test_run_arm_matrix.py` pins the `--skill` and `--model` flags' arm-matrix filtering, preflight arm/job-name selection, and argparse validation, `tests/test_run_completion_gate.py` pins the PASS/INCOMPLETE/FAIL contract and its exit codes, `tests/test_run_prompt_template.py` pins the prompt template (plugin arms still start with their slash command, both arm types carry identical contract text), and `tests/test_run_agent_env.py` pins `AGENT_ENV`'s assembly and its `--ae` emission — that the background-wait ceiling is fixed and unoverridable, that all three `FORWARDED_HOST_ENV` names are forwarded only when the host sets them and are stripped of surrounding whitespace when it does, that a host `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC` is emitted (so it can win at `base.py:320-323`) while an unset one emits nothing (so pier's `"1"` stands), and that no pair is ever emitted with an empty value. `tests/test_triage.py` covers `triage.py` — the retry/record decision — directly and on its own, which it was not always: its rules used to be pinned only in passing by `tests/test_run_scheduled.py`, whose subject is the scheduling loop, and two triage defects shipped under that arrangement (`allowed_warning` read as a quota denial, and `reward == 1` ranked below pier's exception type). It pins the `rate_limit_info.status` vocabulary, worst-fault-wins ordering and its evidence, the `reward == 1` precedence, every branch of `verdict_from_signals`, and the negatives that matter most — a bare `529` in tool output, `overageStatus: "rejected"` inside an *allowed* event, and truncated or non-JSON lines. All of it runs without `pier` and without `runs/`: the recorded event shapes are committed at `tests/fixtures/recorded-rate-limit-events.txt`, and every rule is pinned on those or on staged temp directories. What it deliberately does *not* assert is how the recordings currently under `runs/` happen to triage — how many carry a fault, or which ones. That is a fact about a growing pile of artifacts rather than about a rule: recording one more trial changes the answer without changing any behavior. Two files cover `agent.py`, and only what genuinely needs pier: `tests/test_agent_cost_parsing.py` pins the cost override's resolution order, that the method stayed a shell delegating to `stream_cost`, its file-reading edge cases, and an end-to-end comparison against upstream over the committed copy of the recorded stream; `tests/test_agent_gateway_pins.py` pins the same two properties for the `exec_as_agent` override — that it is what inheritance resolves to, and that it delegates with every parameter preserved. Both skip rather than fail without pier, which is why neither holds a rule of its own.

**Everything else in `run.py`, and the rest of `agent.py`, remains not unit-tested**: assembling the rest of the `pier run` command (only its `--ae` pairs are covered, by `tests/test_run_agent_env.py`), checking out the plugin into a container, driving the container lifecycle. Those are exercised instead by `run.py --dry-run` (prints every command without executing anything) and `run.py --preflight` (actually runs one trial and verifies the plugin loaded and a sub-agent was dispatched). If you change `run.py`'s command-building or `agent.py`'s install steps, `--preflight` — not the test suite — is what tells you whether it still works.
...
| `duration` | Two pacing knobs: `between_runs` (gap after any finished trial, success or failure) and `technical_failure_backoff` (gap before a technical retry) | `1h` and `2h` respectively |
...
| `skips` | Combinations deliberately left unrun, each with a **mandatory** reason and an optional `tasks`/`models`/`skills` selector (omitted = "all") | 3 rules, covering 10 of the 45 expanded cells |
...
**A verifier-certified solve outranks every signal but one.** `triage.verdict_from_signals` checks `reward == 1` before pier's exception type and before the transcript scan, so a trial that solved its task is recorded as a `success` no matter what happened to the process afterwards. The one rule above it is "no trial `result.json` at all": with no result on disk there is no attempt for a verifier to have scored, so that case stays `technical_failure` however a caller arrived at its `resolved` flag — the branch order is what enforces that, not the caller's care. `collect` orders the same two signals the same way (row 1 of the `classify_status` table, decided by `_errored_record` before that function is reached, sits above its `resolved` row 3). That ordering is not hypothetical housekeeping: `runs/do-in-steps__opus-opus__abs-stepped-slices` committed a 131 KB patch, scored `reward 1` / `f2p 1.0` / `p2p 1.0`, and was then killed by an Anthropic 429 on its way out (`NonZeroAgentExitCodeError`) — and with the exception checked first, that verified solve was discarded and re-run three times. `collect.classify_status` applies the same order for the same reason, so `results.json` and `scheduler-state.json` cannot disagree about one trial.

The precedence's one genuinely hard case is deciding *technical* vs. *model failure* when pier reports an abnormal, non-zero process exit with no further explanation (pier's `NonZeroAgentExitCodeError`, which covers everything from a killed container to a crashed `claude` process to a genuine agent bug, all with the same exception type). The transcript is scanned for two concrete signals of an API-side refusal — a `result` event's `api_error_status` set to anything but `null`, or a `rate_limit_event` reporting a denial — and if either fires, the outcome is `technical_failure` regardless of what pier's own exit code said. **When the transcript offers no such evidence, the ambiguous case defaults to `technical_failure` too** (`triage.AMBIGUOUS_NONZERO_EXIT_REASON`, `ambiguous_nonzero_exit`), not to `model_failure`. The two possible mistakes are not symmetric: calling a real model failure "technical" costs one bounded, visible retry — expensive but self-limiting, and once the retry cap is spent the cell is honestly recorded as no data. Calling a real technical failure "a model failure" writes a permanent zero for a trial the model never fairly attempted, and resumability then guarantees it is never revisited — cheap in the moment, and silently corrupts the one number this benchmark exists to produce. So the default absorbs the bounded, visible cost rather than risk the silent, permanent one.

#### What counts as an API-side refusal

`rate_limit_info.status` has three values recorded in this repository's transcripts, and only one of them is a denial:

| `status` | Recorded | Meaning | A fault? |
|---|---:|---|---|
| `allowed` | 14× | The request was served. | No |
| `allowed_warning` | 26× | The request was **also** served — this is a utilization notice, carrying `utilization`/`surpassedThreshold` rather than a denial. claude-code emits one per request once an account crosses a threshold. | No |
| `rejected` | 1× | The request was refused. Observed once, `rateLimitType: "five_hour"`, `overageDisabledReason: "out_of_credits"`, five lines before a `result` event with `api_error_status: 429`. | Yes |
| anything else | 0× | A spelling this harness has not seen. | Yes, at the lowest severity — see below |

The rule is written as "an `allowed`-prefixed status is a served request", not as a list of the two spellings, so a future `allowed_<something>` cannot break it. An *unrecognised* status is deliberately treated as a fault: reading an unknown status as "fine" would write a permanent, invisible zero for a trial that may never have been answered, while reading it as a fault costs one bounded retry and puts the unfamiliar spelling in the state file where an operator will see it. Same asymmetry as `ambiguous_nonzero_exit`, applied to a vocabulary instead of an exit code. Two fields are deliberately **not** markers: `overageStatus: "rejected"`, which appears inside *allowed* events too (keying on it would fail most runs in this repository as quota denials), and any substring search — `529` appears 14–71 times per recorded transcript as source line numbers, uuid fragments and timestamp milliseconds, and never once as an API status.

This is worth spelling out because both halves shipped wrong. The rule used to be "any status other than `allowed`", which made every `allowed_warning` a denial: from the moment an account passed 75% weekly utilization, *every* cell triaged `technical_failure` whatever it had actually done, and since that outcome is never terminal and pier's per-trial resume skips a trial that already has a `result.json`, each retry re-triaged the same stale file — 3 attempts × 2h backoff burned per cell, repeating on every invocation. The scan also returned the *first* marker it found rather than the worst, so the run that really did exhaust its quota was reported by the benign notice on line 6 instead of the denial on line 2936. It now reports the worst fault in the transcript (`rejected` > `api_error_status` > unrecognised status), with the transcript line number and the raw event attached so the reason can be audited instead of re-derived by hand.
...
```json
{
  ...
  "runs": {
    "<task>::<model>::<skill>": {
      ...
      "trial_dir": "runs/…/abs-stepped-slices__fAGX8MS",
      "api_fault": {
        "slug": "rate_limit_status=rejected",
        "severity": 3,
        "line_number": 2936,
        "faults_seen": 2,
        "event": { "…the raw stream event, verbatim…" }
      }
    }
  }
}
...
`trial_dir` and `api_fault` are evidence, written only when there is any — they name which trial was judged and what the transcript scan found there, so this file alone answers "why did this cell end up like that" instead of needing the triage re-run by hand over an 8 MB transcript. Both are additive optional keys, so `version` stays **1**: every reader of this file (`scheduler.load_state`, `collect.load_scheduler_state`) reads named keys and ignores the rest.
...
### `runs/scheduler-attempts.jsonl`

Written beside the state file, one JSON object **appended** per attempt — the deliberate contrast with the state file above, which is rewritten whole. The state file keeps exactly one entry per planned cell, so a cell's third attempt erases the record of its first two, and those are precisely the ones an operator asking "why was this technical?" needs. Each line carries the cell key and `arm_id`, the attempt number and the budget it ran under, the verdict (`outcome`/`reason`), the same `trial_dir`/`api_fault` evidence as above, `slept_seconds` (the backoff paid immediately before that attempt, `0.0` for a first attempt), and a timestamp.

It is instrumentation, not policy: nothing in this harness reads it back, no decision depends on it, and a failure to append one is printed as a `WARN` and otherwise ignored — it does not reach the exit code, because losing a multi-day benchmark over a log line would be an absurd trade. Read it with e.g. `jq -c 'select(.outcome == "technical_failure")' runs/scheduler-attempts.jsonl`.
...
Within one invocation, a technical failure is retried up to `scheduler.MAX_TECHNICAL_RETRIES` times — **2**, i.e. 3 attempts total per cell — waiting `technical_failure_backoff` (2h in the committed file) before each retry. There is no unbounded `while` loop anywhere in this path: the attempt count is a bounded `for` loop, so the total work one schedule can ever do is computable in advance (`len(runnable) * 3` executions, worst case). The bound is set to outlast the window of the only rate-limit *denial* recorded in this repository (`"rateLimitType": "five_hour"`): two 2-hour backoffs plus the run time of the two failed attempts comfortably spans five hours. The transcripts also carry `"rateLimitType": "seven_day"`, but only on `allowed_warning` utilization notices — served requests, not refusals — and a seven-day denial is not something a retry cap could ever outlast anyway; that case is covered by `technical_failure` never being written down as final, so a later invocation picks the cell back up. The bound also caps how much the ambiguous-exit default above can cost — at most two extra executions of a cell, never an unbounded spend, if it turns out to have mis-triaged a genuine, repeated model crash as technical.
