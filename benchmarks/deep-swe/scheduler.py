...
report, sleeping, reading the clock. `run.py` binds those to the real thing;
...
# waiting *for*: every recorded transcript in this repository reports
# `"rateLimitType": "five_hour"`, so the quota window this exists to outlast
# is five hours. Two backoffs of `technical_failure_backoff` (2h each in the
# committed schedule) plus the run time of the two failed attempts spans that
# window comfortably. A third retry would add hours to an already multi-day
# schedule for a fault class two backoffs already cover.
#
...
def write_state(path: Path, runs: dict[str, dict]) -> None:
    ...
    Whole-file rewrite rather than an append log because the file holds one
    entry per planned run -- 45 of them for the committed schedule -- and a
    format an operator can read in full is worth more here than an
...
class _ScheduleRunner:
    ...
    def _resume(self, planned: schedule.PlannedRun) -> RunReport | None:
        ...
        if completed.verdict.is_technical:
            self.log(
                ...
                f"{completed.verdict}; not a fair attempt, so it does not count as done"
            )
            return None

        self.log(
            f"DONE {planned.task.name} / {planned.arm_id} -- {completed.verdict} "
            f"(job dir already complete)"
        )
    ...
    def _execute_with_retries(self, planned: schedule.PlannedRun) -> RunReport:
        ...
        for attempt in range(1, budget + 1):
            ...
            self.log(f"     {report.label} -- {outcome.verdict}")
